...
static const uint8_t rom[ROM_SIZE] PROGMEM = {0};  // ROM: 16KB, chỉ đọc, khởi tạo = 0
static uint8_t ram[RAM_SIZE]; // RAM: 16KB, đọc/ghi, khởi tạo sau bằng memory_init()
...
uint8_t memory_read(uint16_t addr)
{
    if (addr >= MEM_ROM_START && addr <= MEM_ROM_END)
		return pgm_read_byte(&rom[addr - MEM_ROM_START]); // Đọc từ ROM sử dụng pgm_read_byte vì ROM được lưu trữ trong bộ nhớ chương trình (flash)
